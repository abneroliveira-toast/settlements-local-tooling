import { schema } from "../src/dynamodb-tables/ppc.js";
    ...
    try {
      ...
      for (i = 0; i < resultDynamoList.length; i++) {
        ...
        result[mid] = {
          ...
          vendor: mids[i].vendor,
        };
      }
      console.log("Loop: " + i);
      await delay(1000);
      // const fileNameResult = path.join(process.env.PWD, `data/result-${i}.json`);
      // fs.writeFileSync(fileNameResult, JSON.stringify(result));
    } catch (err) {
      console.log("Error", err);
    }
  }
  const fileNameResult = path.join(process.env.PWD, `./output/mids.json`);
  fs.writeFileSync(fileNameResult, JSON.stringify(result));
...
async function getSubmerchantsIdsInCSV() {
  return new Promise((resolve, reject) => {
    const filePath = path.resolve("./dynamo-scripts/submerchant_ids.csv");
